...
class DeepQLearning:
    ...
    def __init__(self, env, gamma, epsilon, epsilon_min, epsilon_dec, episodes, batch_size, memory, model):
        ...
        self.model_secondary = model

    def select_action(self, state):
        if np.random.rand() < self.epsilon:
            return random.randrange(self.env.action_space.n)
        action = self.model_secondary.predict(state)
    ...
    def experience_replay(self, episode):
        if len(self.memory) > self.batch_size:
            ...
            self.model_primary.fit(states, targets_full, epochs=1, verbose=0)

            if episode % 10 == 0:
                self.model_secondary.set_weights(self.model_primary.get_weights())
